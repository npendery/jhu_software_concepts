...
def clean_data(raw_data_list):
    ...
    for raw_entry in raw_data_list:
        entry = {
            ...
            "program_season_year": raw_entry.get(
                "semester_and_year_of_program_start_raw", unknown_value
            ),
        ...
        applicant_status, decision_date = _parse_decision_info(
            raw_entry.get("decision_info_raw", unknown_value)
        )
        entry["applicant_status"] = applicant_status
        entry["decision_date"] = decision_date

        gre_total = entry.get("gre_total", unknown_value)
        if isinstance(gre_total, str):
            gre_total = _remove_label(gre_total, r"GRE V*(AW)*")
...
def _parse_decision_info(text):
    """Parses decision info from a string such as 'Accepted on 17 Apr'."""
    match = re.search(r"(.*)\s(?=on\s)(?:on)\s(.*)", text, re.IGNORECASE)
    if match:
