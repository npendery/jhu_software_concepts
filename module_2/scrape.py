...
def _is_first_row(entry_row):
    """Check if the entry is the first row of an entry group by checking for a See More link."""
    return entry_row.find('a', string=re.compile(r"See More", re.IGNORECASE)) is not None


def _is_second_row(entry_row):
    """Check if the entry is the second row of an entry group by checking for a div with the tw-bg-orange-400 class."""
    return entry_row.find('div', class_='tw-bg-orange-400') is not None


def _is_third_row(entry_row):
    """Check if the entry is the third row of an entry group by checking for a p with the tw-text-gray-500 class."""
...
if __name__ == "__main__":
    ...
    if raw_data:
        ...
        print(f"Successfully saved {len(raw_data)} raw entries to raw_applicant_data.json")
    else:
