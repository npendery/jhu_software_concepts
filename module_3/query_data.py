...
def question_4_avg_gpa_american_fall_2024():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    query = "SELECT AVG(gpa) FROM applicants WHERE us_or_international = 'American' AND term = 'Fall 2024' AND gpa IS NOT NULL;"
    cur.execute(query)
...
def question_5_fall_2024_acceptance_percentage():
    ...
    query_acceptances = "SELECT COUNT(*) FROM applicants WHERE term = 'Fall 2024' AND status = 'Accepted';"
    cur.execute(query_acceptances)
    acceptance_count = cur.fetchone()[0]

    query_total_fall_2024 = "SELECT COUNT(*) FROM applicants WHERE term = 'Fall 2024';"
...
def question_6_avg_gpa_accepted_fall_2024():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    query = "SELECT AVG(gpa) FROM applicants WHERE term = 'Fall 2024' AND status = 'Accepted' AND gpa IS NOT NULL;"
    cur.execute(query)
    avg_gpa = cur.fetchone()[0]
    cur.close()
    conn.close()
...
def question_7_jhu_cs_masters_entries():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    query = "SELECT COUNT(*) FROM applicants WHERE program ILIKE '%Johns Hopkins%' AND program ILIKE '%Computer Science%' AND degree = 'Masters';"
    cur.execute(query)
    count = cur.fetchone()[0]
    cur.close()
    conn.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Please ensure your PostgreSQL server is running, credentials are correct, and the table exists with data."
        )
